//! NFSv4.2 Tokio cookbook.
//!
//! Run against a writable path in the server's v4 pseudo-filesystem:
//!
//! ```text
//! cargo run --example v4_tokio_cookbook --features tokio -- 127.0.0.1 /export/writable-dir
//! ```
//!
...
const OBJECT: &[u8] = b"async nfs-rs v4 cookbook object\nline two\nline three\n";
const STREAMED: &[u8] = b"streamed through async NFSv4 open/write/close\n";

#[tokio::main(flavor = "current_thread")]
async fn main() -> nfs::Result<()> {
    let config = Config::from_env();
    let mut client = connect(&config.host).await?;

    println!("connected to async NFSv4 host {}", config.host);
    ...
    let work_dir = remote_path(
        &config.remote_dir,
        &format!("nfs-rs-v4-tokio-cookbook-{}", std::process::id()),
    );
    client.remove_all_if_exists(&work_dir).await?;
    client.create_dir_all(&work_dir, 0o755).await?;

    let flow_result = run_filesystem_flow(&mut client, &work_dir).await;
    let cleanup_result = client.remove_all_if_exists(&work_dir).await;
    let shutdown_result = client.shutdown().await;
    finish(flow_result, cleanup_result, shutdown_result)
}

async fn connect(host: &str) -> nfs::Result<Client> {
    let process_id = std::process::id();
    Client::builder(host.to_owned())
        .auth_sys(AuthSys::current())
        .timeout(Some(Duration::from_secs(10)))
        .port(2049)
        .owner_id(format!("nfs-rs:v4-tokio-cookbook:{host}:{process_id}").into_bytes())
        .open_owner(format!("nfs-rs:v4-tokio-open-owner:{process_id}").into_bytes())
        .io_size(128 * 1024)
        .dir_size(64 * 1024)
...
async fn run_filesystem_flow(client: &mut Client, work_dir: &str) -> nfs::Result<()> {
    ...
    client
        .write_atomic_with_mode(&object, OBJECT, 0o640)
        .await?;
    print_v4_metadata(
        "object after atomic write",
        &client.metadata(&object).await?,
    );

    let exists = client.exists(&object).await?;
    ...
    let first_five = client.read_exact_at(&object, 0, 5).await?;
    println!(
        "first five bytes: {:?}",
        String::from_utf8_lossy(&first_five)
    );

    let middle = client.read_range(&object, 7, 14).await?;
    println!("range read: {:?}", String::from_utf8_lossy(&middle));

    let mut sink = tokio::io::sink();
    let downloaded_len = client.read_to_writer(&object, &mut sink).await?;
    println!("streamed object into async writer: {downloaded_len} bytes");

    let mut upload = STREAMED;
    let uploaded_len = client
        .write_atomic_from_reader_with_mode(&streamed, &mut upload, 0o644)
        .await?;
    println!("streamed upload wrote {uploaded_len} bytes");

    client.append(&streamed, b"appended\n").await?;
    client.write_at(&streamed, 0, b"STREAMED").await?;
    client.truncate(&streamed, 24).await?;
    let commit = client.commit(&streamed, 0, 0).await?;
    println!("commit verifier: {:02x?}", commit.verifier);

    client.create_new_with_mode(&created, 0o600).await?;
    client
        .write_at(&created, 0, b"created with create_new_with_mode\n")
        .await?;
    let _ = optional(
    ...
    let copied_len = client.copy_atomic(&streamed, &copied).await?;
    println!("copied {copied_len} bytes from {streamed} to {copied}");

    client.rename(&copied, &renamed).await?;
    let _ = optional("touch", client.touch(&renamed).await);
    let _ = optional("set_mode", client.set_mode(&renamed, 0o644).await);

    client.create_dir_all(&nested_dir, 0o755).await?;
    client.write_atomic(&nested_file, b"nested file\n").await?;

    demonstrate_optional_links(client, &object, work_dir).await?;
    demonstrate_v4_sparse_file_ops(client, &object).await;
    demonstrate_directory_reads(client, work_dir).await?;
    demonstrate_filesystem_queries(client, work_dir).await?;
    demonstrate_error_handling(client, work_dir).await;
    demonstrate_reconnect(client, work_dir).await?;

    client.renew().await?;
    Ok(())
}

async fn demonstrate_optional_links(
    client: &mut Client,
    object: &str,
    work_dir: &str,
) -> nfs::Result<()> {
    let symlink = remote_path(work_dir, "object.symlink");
    if optional("symlink", client.symlink(&symlink, "object.txt").await).is_some() {
        let target = client.read_link(&symlink).await?;
        println!("symlink target: {target}");
    }

    let hard_link = remote_path(work_dir, "object.hardlink");
    if optional("hard_link", client.hard_link(object, &hard_link).await).is_some() {
...
async fn demonstrate_v4_sparse_file_ops(client: &mut Client, object: &str) {
    if let Some(offset) = optional("seek_data", client.seek_data(object, 0).await) {
        println!("first data offset: {offset:?}");
    }
    if let Some(offset) = optional("seek_hole", client.seek_hole(object, 0).await) {
        println!("first hole offset: {offset:?}");
    }
    if optional("allocate", client.allocate(object, 0, 4096).await).is_some() {
        let _ = optional("deallocate", client.deallocate(object, 2048, 1024).await);
    }
}

async fn demonstrate_directory_reads(client: &mut Client, work_dir: &str) -> nfs::Result<()> {
    println!("limited directory listing:");
    for entry in client.read_dir_limited(work_dir, 16).await? {
    ...
    loop {
        let page = client.read_dir_page_limited(work_dir, cursor, 4).await?;
...
async fn demonstrate_filesystem_queries(client: &mut Client, work_dir: &str) -> nfs::Result<()> {
    let supported = client.supported_attrs(work_dir).await?;
    println!(
...
async fn demonstrate_error_handling(client: &mut Client, work_dir: &str) {
    let missing = remote_path(work_dir, "does-not-exist.txt");
    match client.read(&missing).await {
...
async fn demonstrate_reconnect(client: &mut Client, work_dir: &str) -> nfs::Result<()> {
    client.reconnect().await?;
    println!(
        "reconnected; workspace still exists = {}",
        client.exists(work_dir).await?
    );
    Ok(())
}
